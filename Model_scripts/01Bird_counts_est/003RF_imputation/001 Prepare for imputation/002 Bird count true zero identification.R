...
Misdf_list <- lapply(files, function(file) read.csv(file))
...
for (sp in seq_along(1:5)){
  Misdf_list[[sp]]$Seasonal.week <- as.numeric(Misdf_list[[sp]]$Seasonal.week)
  Misdf_list_2[[sp]] <- Misdf_list[[sp]] %>% 
  ...
  Misdf_list_2[[sp]]$isoweek <- as.integer(Misdf_list_2[[sp]]$isoweek)
  Misdf_list_2[[sp]]$isoyear <- as.integer(Misdf_list_2[[sp]]$isoyear) 
  
  Misdf_list_2[[sp]] <- left_join(df_struc, Misdf_list_2[[sp]], by=c('KN10kmDK', 'isoweek', 'isoyear'))
}
Misdf_list <- Misdf_list_2

WaterbirdTrip <- read.csv('Data/Anatidae trips per 10x10 km square per day 2016-2021.csv', sep=";")
...
a_agg <- right_join(a_agg, Misdf_list[[1]] %>% mutate(time=isoyear*100+isoweek), by=c('KN10kmDK', 'time')) %>% select(KN10kmDK, time, anatidae_trips)
a_agg$anatidae_trips <- if_else(is.na(a_agg$anatidae_trips), 0, ceiling(a_agg$anatidae_trips))
list_name <- c('WS', 'BG', 'MS', 'GG', 'M')

#5 species
zerocheck <- list(); tmp<-list(); count_trip <- list(); finalMisdf_list <- list()
for(sp in 1:5) {
  zerocheck[[sp]] <- Misdf_list[[sp]] %>% select(KN10kmDK, DOF.ratio.counts, isoyear, isoweek) %>%  group_by(isoyear, isoweek) %>% group_by(KN10kmDK) %>% mutate(isoyear=as.numeric(as.character(isoyear)), isoweek=as.numeric(isoweek)) %>% mutate(time= isoyear*100+isoweek)
  ...
  finalMisdf_list[[sp]] <- left_join(zerocheck[[sp]], count_trip[[sp]], by=c("KN10kmDK", "time")) %>% select(KN10kmDK, time, DOF.ratio.counts, zerotype) %>% mutate(counts = case_when(
    zerotype==1 & DOF.ratio.counts==0 ~ 0,
    zerotype==0 & DOF.ratio.counts==0 ~ NA,
    TRUE ~ DOF.ratio.counts
  ))
  write.csv(finalMisdf_list[[sp]], paste0("Data/", list_name[sp], "full_idenTrue0.csv"))
}
