...
compartment_sf <- readRDS("Data/DanHPAIwild params/compartment_sf.rds")
V_disp <- lapply(compartment_sf, function(x) x$S)
source(".../DanHAPIwildModel/Model_scripts/02DanHPAIwild_build/Inputs/Truncated_rnultinom function.R")
theta_A_list <- readRDS("Data/DanHPAIwild params/list of theta_A_raw_2ConseYear.rds")
passur <- read.csv("Data/DanHPAIwild params/passur_species_matrix.csv", sep = ";")
passur <- as.matrix(passur[, -1])
#Proportion of water areas by cell
CLC <- read.csv("H:/All/Backup_documents/KU-PhD_030622/Phd Project/PhD plan/Manuscript 2/DanHPAIWild/DanHAPIwildModel/Data/Bird counts/Bird counts raw/GLMM/DKLine1kmCLC proportions.csv")
xi <- CLC$Water.bodies+CLC$Wetlands

#load disp_prob 
source("H:/All/Backup_documents/KU-PhD_030622/Phd Project/PhD plan/Manuscript 2/DanHPAIWild/DanHAPIwildModel/Model_scripts/02DanHPAIwild_build/Inputs/Dispersal probability generation.R")
...
save(S_list_A, I_list_A, dD_list_A, D_list_A, Rem_list_A, Rec_list_A, dRem_list_A, V_disp_A, prob_A, V_M, disp_prob_L, tmp_S_list_A, tmp_I_list_A, tmp_Rec_list_A, dRec_list_A, theta_A_list, passur, N_Env, contact, ID50, gamma, mu, mf, eta, epsilon, sigma, foraging, xi, file="Data/DanHPAIwild params/DanHPAIwild parameters_Raw.RData")
