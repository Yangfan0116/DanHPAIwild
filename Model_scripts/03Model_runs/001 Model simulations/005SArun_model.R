...
#Peak values are equal to the weight ratios, when we assume the peak contact rate of mallards is 1. The contact rates peak at the beginning of Oct. in the baseline model. 
WS <- function(x) 5 * (1 + cos(2 * pi / 52 * x))
BG <- function(x) 1 * (1 + cos(2 * pi / 52 * x))
MS <- function(x) 5.5 * (1 + cos(2 * pi / 52 * x))
GG <- function(x) 1.5 * (1 + cos(2 * pi / 52 * x))
M <-  function(x) 0.5 * (1 + cos(2 * pi / 52 * x))
...
session <- sessionInfo()
save(results, session, file = "H:/All/Backup_documents/KU-PhD_030622/Phd Project/PhD plan/Manuscript 2/Submission/DanHPAIwild/DanHPAIwild/Model_scripts/03Model_runs/003 Simulation outputs/005contact model output.rda")
